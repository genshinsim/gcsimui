<script>
  ...
  const handleLoadTeam = () => {
    const next = teamToConfig($charStore);
    console.log(next);
    teamConfigStore.set(next);
  };

  const handleClearTeamConfig = () => {
    teamConfigStore.set("");
  };
  ...
  const handleRun = () => {
    ...
    // const testcmd = new Command("echo", "hi");
    // testcmd.execute().then((msg) => console.log(msg));
    ...
    // invoke("run_sim_from_js", { simConfig: cfg }).then((message) => {
    //   // console.log(message);
    //   //close the existing spinner
    //   close();
    //   const result = JSON.parse(message);
    //   if (result.err) {
    //     //pop open another modal displaying the error
    //     open(Error, { err: result.err });
    //     return;
    //   }
...
<div class="flex flex-col">
  <div class="pl-6 pr-6">
    <div class="text-lg font-medium pt-2 pb-2">Team Config</div>
    <div class="flex flex-row gap-2 pl-3">
      <textarea
        spellcheck="false"
        rows="8"
        class="bg-gray-700 resize-none rounded-md whitespace-pre text-xs font-mono w-full"
        placeholder="Enter team config here (or leave blank and enter it all in box below)"
        bind:value={$teamConfigStore}
      />
      <div class="flex flex-col gap-2">
        <button class="btn btn-primary w-48" on:click={handleLoadTeam}
          >Load from Builder</button
        >
        <button class="btn btn-accent w-48" on:click={handleClearTeamConfig}
          >Clear</button
        >
      </div>
    </div>
    <div class="mt-2">
      <div class="text-lg font-medium pt-2 pb-2">Actions</div>
      <div class="flex flex-row gap-2 pl-3">
        <textarea
          spellcheck="false"
          rows="20"
        ...
        />
        <div class="flex flex-col gap-2">
          <!-- <button class="btn btn-primary w-48">Load Template</button>
          <button class="btn btn-primary w-48">Load from Builder</button> -->
          <button class="btn btn-accent w-48" on:click={handleClearActionConfig}
            >Clear</button
          >
        </div>
      </div>
    ...
    <div class="w-full flex flex-row gap-4 mt-4 p-4">
      <div>
        <button class="btn btn-primary w-48" on:click={handleRun}>
          <svg
            aria-hidden="true"
            focusable="false"
            data-prefix="fas"
      ...
      <div>
        <button class="btn btn-secondary w-48" on:click={handleOpenOpts}>
          <svg
            aria-hidden="true"
            focusable="false"
            data-prefix="fas"
